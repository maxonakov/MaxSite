<?php
/**
 * Template part for displaying footer social icons
 *
...
if ( $social_lists ) :
    ...
    $inner_wrapper_class = ['d-flex flex-wrap social-list footer-social-inner social-icons'];
    ?>
        ...
        <ul class="<?php echo esc_attr( implode( ' ', $inner_wrapper_class ) ); ?>">
            <?php foreach ( $social_lists as $index => $values ) : ?>
                <?php if (!empty($values['social_type'])) :
                    ...
                    $icon_position  = get_theme_mod('social_network_icon_position','flex-row');
